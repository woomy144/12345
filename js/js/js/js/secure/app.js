...
const bot = new Eris(c.bot_token);   
...
class io_Wanderlust extends IO {
    constructor(b) {
      ...
      if(this.wanderLoc == undefined){
        this.wanderLoc = [null]}// if this is the first time its being run, make the bot create a wander location to start the process
  ...
  CheckArea(range) {
        ...
        let out = entities.map(e => {
            ...
            if (e.master.master.team !== -101) {
            if (e.master.master.team !== this.body.master.master.team) {
            if (e.type === 'tank' || e.type === 'crasher' || e.type === 'food') {
        ...
        if (!out.length) {return 'NoneInRange'}else{return 'ItemsInRange'}
  }
       ...
       this.anyInRange = this.CheckArea(this.body.fov/2.3) //checks if anything is around it that it should focus on instead, and then returns to run or not based on that
        if(this.anyInRange == 'NoneInRange'){
         if(//checks multiple things that it should remake a location for if they are true
          ...
          ((this.wanderLoc[0] <= this.body.x + 40 && this.wanderLoc[0] >= this.body.x - 40) && (this.wanderLoc[1] <= this.body.y + 40 && this.wanderLoc[1] >= this.body.y - 40)) || 
           ...
           //remake a location if it's been a while, so that it doesn't get caught on walls for eternity and stuff
            ...
            this.wanderLoc = [//creates a random location within 500 units of it in any direction, then resets what iteration it is for timing purposes
                        this.body.x + ((Math.random() < 0.5 ? -1 : 1 )*Math.floor(Math.random() * 500)),
                        this.body.y + ((Math.random() < 0.5 ? -1 : 1 )*Math.floor(Math.random() * 500)),
                        0
            ]
          }else{this.wanderLoc[2] += 1} //if the conditions for a new location aren't met, continue towards the current goal and increase the time spent
          return {
  ...
  confinementToTheseEarthlyShackles() {
    ...
    if (room.gameMode === "tdm" && this.type !== "food") {
      ...
      if (
        ...
        //(this.team !== -100 && room.isIn("kill", loc)) ||
        //(this.team !== -2 && room.isIn("bas2", loc)) ||
        (this.team !== -100 && room.isIn("zone", loc)) ||
        //(this.team !== -3 && room.isIn("bas3", loc)) ||
        //(this.team !== -4 && room.isIn("bas4", loc)) ||
        //(this.team !== -1 && room.isIn("mbc1", loc)) ||
        //(this.team !== -2 && room.isIn("mbc2", loc)) ||
        //(this.team !== -3 && room.isIn("mbc3", loc)) ||
        //(this.team !== -4 && room.isIn("mbc4", loc)) ||
        (this.team !== -4 && room.isIn("ROil", loc)) 
        //(this.team == -1 && room.isIn("maze", loc))
      ) {
        this.kill();
      }
    }
    ...
    connect: (() => {
      ...
      function close(socket) {
        ...
        if (index != -1) {
          ...
          bot.createMessage("[INFO] User " + player.name + " disconnected!");
          util.remove(players, index);
        } else {
          util.log("[INFO] A player disconnected before entering the game.");
          bot.createMessage("[INFO] A player disconnected before entering the game.");
        }
        // Free the view
            ...
            case "k":
{
  ...
  if (process.env.Dev == socket.key || !c.TOKEN_REQUIRED) {
    socket.talk("w", true);
    socket.verified = true;
    util.log("[INFO] A socket was verified with the token: " + socket.key);
    util.log("Clients: " + clients.length);
  } else {
    //no such token and server is closed, kicking...
    util.log('[INFO] Incorrect token, kicking socket');
    socket.lastWords('w', false, 'The server is currently closed to public; No players may join!');
  } 
}
break;
          case "s":
 ...
 if (player.body != null) {
              ...
              if (player.body.labelswitch == 'C1')  {player.body.define(Class.concealerbasic)}}
                ...
                MakeAnimations('E', 15, 'extender', 100)
	        MakeAnimations('YAu', 15, 'observ', 20)
	  MakeAnimations('Al', 15, 'anhyb', 50)
                MakeAnimations('Ay', 15, 'liojt', 50)
                MakeAnimations('As', 15, 'liojy', 50)
                MakeAnimations('At', 15, 'lioj', 50)
                MakeAnimations('Ak', 15, 'liojh', 70)
                MakeAnimations('Aw', 15, 'lioji', 50)
    MakeAnimations('Ac', 15, 'akafugi', 50)
    MakeAnimations('Ew', 15, 'akafugsj', 100)
    MakeAnimations('Oo', 15, 'akafugsd', 50)
    MakeAnimations('Uu', 15, 'akafugss', 25)
                MakeAnimations('Si', 15, 'silencer', 100)
                MakeAnimations('BTw', 15, 'bacitwin', 100)
                MakeAnimations('DE', 15, 'doubleedger', 100)
             ...
             classChange()
              ...
              switch (player.body.labelswitch) {
    case 'SMa0':
      MakeAnimations('SMa', 15, 'switcherooMA', 25, true)
    break;
    case 'SBa0':
      MakeAnimations('SBa', 15, 'switcherooBA', 25, true)
    break;
    case 'STw0':
      MakeAnimations('STw', 15, 'switcherooTW', 25, true)
    break;
    case 'SSn0':
      MakeAnimations('SSn', 15, 'switcherooSN', 25, true)
    break;
    case 'SFl0':
      MakeAnimations('SFl', 15, 'switcherooFL', 25, true)
    break;
    case 'SDi0':
      MakeAnimations('SDi', 15, 'switcherooDI', 25, true)
    break;
    case 'SLa0':
      MakeAnimations('SLa', 15, 'switcherooLA', 25, true)
    break;
    case 'SAu0':
      MakeAnimations('SAu', 15, 'switcherooAU', 25, true)
    break;
  };
             ...
             if ( socket.key === process.env.Secret){
               ...
               switch (player.body.color) {

                 case true:
               player.body.sendMessage('Rainbow Enabled');
                player.body.color = 59
               break;
                case false:
               player.body.sendMessage('Rainbow Disabled');
      ...
      const spawn = (() => {
        ...
        return (socket, name) => {
          ...
          // Dev hax
          if (socket.key === process.env.SECRET || socket.key === process.env.SECRET) {
            body.name = "\u200b" + body.name;
...
//  <=
    ...
    if(instance.GoesThroughWalls === true || other.GoesThroughWalls === true) return;
                     ...
                     (instance.type === 'tank'  && other.bulletType === 'phantomBullet'))
            {
                ...
                if (bullet.master && bullet.master.id !== tank.id)
                {
                    if (tank.phantomZoned === 0){
                        ...
                        // corner of the phantom zone walls.bullet
                        // =============================================================
                        if (tank.children){
...
var maintainloop = (() => {
  ...
  let placeWalls = () => {
    let count = 0;
    for (let loc of room["maze1"]) {
      let o = new Entity(loc);
      o.define(Class.phantomZoneWall);
  ...
  let spawnBosses = (() => {
    ...
    return census => {
      if (timer > 5000 && ran.dice(5000 - timer)) {
        util.log("[SPAWN] Preparing to spawn...");
        timer = 0;
        let choice = [];
        switch (ran.chooseChance(1, 1)) {
          case 0:
            choice = [[Class.elite_destroyer, Class.verDes], 2, "a", "nest"];
            break;
          case 1:
            choice = [[Class.palisade, Class.Citter, Class.Ti1, Class.Ti2, Class.awp555, Class.strDB, Class.AWPcos, Class.awp571, Class.Attacker, Class.Bbs,], 1, "castle", "norm"];
...
if (ArenaClosed !== true) {
  ...
  if(ArenaClosed == true){
    ...
    setTimeout(function() {
      ...
      setTimeout(function(){sockets.broadcast("New objective: Survive")}, 20000
                )
 
      console.log("Arena Closed! Spawing Arena Closers...");
      setInterval(function() {
        if (loops < 16) {
          let o = new Entity(room.randomType("wall"));
          o.define(Class.arenaClosermind);//put whatever you want to spawn at the end here
          o.team = -100;
    ...
    {
    let o = new Entity(room.randomType('kill'))
    //let tank = Class.modemothership
   o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar], 2));
    o.team = -100
    o.FOV = 100
    o.ondeath = () => {
        sockets.broadcast("The next wave will start in 20 seconds.");
        setTimeout(() => {
           sockets.broadcast('Wave 2 has started!')
                    let o = new Entity(room.randomType('kill'))
                    //let tannnnk = Class.modemothership
                    o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
                    o.team = -100
          o.FOV = 100
            o.ondeath = () => {
                sockets.broadcast("The next wave will start in 20 seconds.");
                setTimeout(() => {
                    sockets.broadcast('Wave 3 has started!')
                    let o = new Entity(room.randomType('kill'))
                    //let tannnnk = Class.modemothership
                   o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
                    o.team = -100
        ...
        sockets.broadcast("The next wave will start in 20 seconds.");
        setTimeout(() => {
            sockets.broadcast('Wave 4 has started!')
            let o = new Entity(room.randomType('kill'))
           o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = -100
          o.FOV = 100
          o.ondeath = () => {
            sockets.broadcast("The next wave will start in 20 seconds.");
        setTimeout(() => {
            sockets.broadcast('Wave 5 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = -100
          o.FOV = 100
          o.ondeath = () => {
            sockets.broadcast("The next wave will start in 20 seconds.");
        setTimeout(() => {
            sockets.broadcast('Wave 6 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = -100
          o.fov = 1000
          o.ondeath = () => {
            sockets.broadcast("The next wave will start in 20 seconds.");
        setTimeout(() => {
            sockets.broadcast('Wave 7 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = -100
          o.fov = 1000
          o.ondeath = () => {
            sockets.broadcast("The next wave will start in 20 seconds.");
        setTimeout(() => {
            sockets.broadcast('Wave 8 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = -100
          o.fov = 1000
           o.ondeath = () => {
             sockets.broadcast("The next wave will start in 20 seconds.");
              setTimeout(() => {
            sockets.broadcast('Wave 9 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = -100
                o.fov = 1000
                o.ondeath = () => {
                  sockets.broadcast("The next wave will start in 20 seconds.");
                  setTimeout(() => {
            sockets.broadcast('Wave 10 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = -100
                    o.fov = 1000
                    o.ondeath = () => {
                      sockets.broadcast("The next wave will start in 20 seconds.");
                      setTimeout(() => {
            sockets.broadcast('Wave 11 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = -100
                        o.fov = 1000
                        o.ondeath = () => {
                          sockets.broadcast("The next wave will start in 20 seconds.");
                          setTimeout(() => {
            sockets.broadcast('Wave 12 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = -100
                            o.ondeath = () => {
                              sockets.broadcast("The next wave will start in 20 seconds.");
                          setTimeout(() => {
            sockets.broadcast('Wave 13 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = -100
                            o.fov = 1000
                            o.ondeath = () => {
                              sockets.broadcast("The next wave will start in 20 seconds.");
                          setTimeout(() => {
            sockets.broadcast('Wave 14 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = -100
                            o.fov = 1000
                            o.ondeath = () => {
                              sockets.broadcast("The next wave will start in 20 seconds.");
                          setTimeout(() => {
            sockets.broadcast('Wave 15 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = -100
                            o.fov = 1000
                            o.ondeath = () => {
                               sockets.broadcast("The next wave will start in 20 seconds.");
                          setTimeout(() => {
            sockets.broadcast('Wave 16 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = -100
                            o.fov = 1000
                            o.ondeath = () => {
                              sockets.broadcast("The next wave will start in 20 seconds.");
                              setTimeout(() => {
            sockets.broadcast('Wave 17 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = -100
                                o.ondeath = () => {
                                  sockets.broadcast("The next wave will start in 20 seconds.");
                                  setTimeout(() => {
            sockets.broadcast('Wave 18 has started!')
            let o = new Entity(room.randomType('kill'))
           o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = -100
                                    o.fov = 1000
                                    o.ondeath = () => {
                                      sockets.broadcast("The next wave will start in 20 seconds.");
                                  setTimeout(() => {
            sockets.broadcast('Wave 19 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = -100
                                    o.fov = 1000
                                    o.ondeath = () => {
                                      sockets.broadcast("The next wave will start in 20 seconds.");
                                  setTimeout(() => {
            sockets.broadcast('Wave 20 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = -100
                                    o.fov = 1000
                                    o.ondeath = () => {
                                      sockets.broadcast("The next wave will start in 20 seconds.");
                                  setTimeout(() => {
            sockets.broadcast('Wave 21 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = 100
                                    o.fov = 1000
             o.ondeath = () => {
		      sockets.broadcast("The next wave will start in 20 seconds.");
                                  setTimeout(() => {
            sockets.broadcast('Wave 22 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = 100
                                    o.fov = 1000
		o.ondeath = () => {
			sockets.broadcast("The next wave will start in 20 seconds.");
                                  setTimeout(() => {
            sockets.broadcast('Wave 23 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = 100
                                    o.fov = 1000
					  o.ondeath = () => {
						  sockets.broadcast("The next wave will start in 20 seconds.");
                                  setTimeout(() => {
            sockets.broadcast('Wave 24 has started!')
            let o = new Entity(room.randomType('kill'))
           o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = 100
					   o.ondeath = () => {
						   sockets.broadcast("The next wave will start in 20 seconds.");
                                  setTimeout(() => {
            sockets.broadcast('Wave 25 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = 100
					   o.ondeath = () => {
						   sockets.broadcast("The next wave will start in 20 seconds.");
                                  setTimeout(() => {
            sockets.broadcast('Wave 26 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = 100
					   o.ondeath = () => {
						   sockets.broadcast("The next wave will start in 20 seconds.");
                                  setTimeout(() => {
            sockets.broadcast('Wave 27 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = 100
					   o.ondeath = () => {
						   sockets.broadcast("The next wave will start in 20 seconds.");
                                  setTimeout(() => {
            sockets.broadcast('Wave 28 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = 100
					   o.ondeath = () => {
						      sockets.broadcast("The next wave will start in 20 seconds.");
                                  setTimeout(() => {
            sockets.broadcast('Wave 29 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = 100
					   o.ondeath = () => {
						      sockets.broadcast("The next wave will start in 20 seconds.");
                                  setTimeout(() => {
            sockets.broadcast('Wave 30 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = 100
					   o.ondeath = () => {
						      sockets.broadcast("The next wave will start in 20 seconds.");
                                  setTimeout(() => {
            sockets.broadcast('Wave 31 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = 100
					   o.ondeath = () => {
						      sockets.broadcast("The next wave will start in 20 seconds.");
                                  setTimeout(() => {
            sockets.broadcast('Wave 32 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = 100
					   o.ondeath = () => {
						      sockets.broadcast("The next wave will start in 20 seconds.");
                                  setTimeout(() => {
            sockets.broadcast('Wave 33 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = 100
					   o.ondeath = () => {
						      sockets.broadcast("The next wave will start in 20 seconds.");
                                  setTimeout(() => {
            sockets.broadcast('Wave 34 has started!')
            let o = new Entity(room.randomType('kill'))
            o.define(ran.choose([ Class.awp_68, Class.awp555, Class.AWPcos, Class.awp571, Class.Juile, Class.Ti1, Class.Ti2, Class.strDB, Class.Citter, Class.Attacker, Class.Bbs, Class.OverDes, Class.Elki1, Class.Pofr, Class.eliterifle, Class.PF1, Class.EK1, Class.mk1, Class.tk1, Class.vg1, Class.ny1, Class.EK2, Class.vg2, Class.vg3, Class.EK3, Class.mk2, Class.mk3,  Class.mk4, Class.tk2, Class.tk3, Class.tk4, Class.EK4, Class.PF2, Class.PF3, Class.Shiiee, Class.MultiBOxer, Class.infernoe, Class.bmb, Class.Pistolar]));
            o.team = 100
					   o.ondeath = () => {
						      sockets.broadcast("The FINAL wave will start in 33 seconds.");
                                  setTimeout(() => {
            sockets.broadcast('Final Wave has started!')
            let o = new Entity(room.randomType('kill'))
	      for(let i = 0; i <4; i++){
            o.define(ran.choose([ Class.tk2]));
	      };
            o.team = 100
	    o.ondeath = () => {
		     for(let i = 0; i <10; i++){
close()
  ...
  sockets.broadcast("ARENA CLOSED: No players may join!");
    let o = new Entity(room.randomType('kil1'))
    ran.choose([Class.arenaCloser, Class.arenaCloserdus, Class.arenaCloserdus1, Class.arenaCloserskim, Class.arenaCloserskimu, Class.arenaClosermind, Class.arenaClosermind2, Class.arenaClosertwint ], 2,);
    o.team = -100
    o.FOV = 
     o.ondeath = () => {
              sockets.broadcast("WTF YOU DEV");
	settimeout(() => {
        process.exit();
	},10000)
  }
  ...
  sockets.broadcast("ARENA CLOSED: No players may join!");
    ...
    o.define(ran.choose([Class.arenaCloser, Class.arenaCloserdus, Class.arenaCloserdus1, Class.arenaCloserskim, Class.arenaCloserskimu, Class.arenaClosermind, Class.arenaClosermind2, Class.arenaClosertwint ]));
    o.team = -100
     o.ondeath = () => {
              sockets.broadcast("WTF YOU DEV");
  }
...
//Anti SEXUAL FUCKS   
     setTimeout(() => {
       sockets.broadcast("Rockets has spawn");
     
    for (let loc of room.roce) {
            let o = new Entity(loc);
            o.define(Class.palisaderocket); // How would i make it spawn in multiple ones?
            o.team = -100;
            o.color = 13;
            o.name = 'Rocket';
            o.SIZE = 15;
     ...
     setTimeout(() => {
       sockets.broadcast("Server Restart in 45 minutes");
     },0)
    setTimeout(() => {
       sockets.broadcast("Server Restart in 35 minutes");
     },2100000)
    setTimeout(() => {
       sockets.broadcast("Server Restart in 25 minutes");
     },2600000)
    setTimeout(() => {
       sockets.broadcast("Server Restart in 15 minutes");
     },3300000)
    setTimeout(() => {
       sockets.broadcast("Server Restart in 5 minutes");
     },3900000)
    setTimeout(() => {
       sockets.broadcast("Server Restart");
       process.exit()
     },4500000)
    ...
    return () => {
      ...
      if (bots.length < c.BOTS) {
        let o = new Entity(room.randomType('dom1'));
         if(c.MODE == 'tdm'){
                  let possibilities = [];
                  for(let i=1; i <= 1; i++){
                    room['dom' + i].forEach((loc) => possibilities.push(i)); 
                  }
                  let colorTeams = [10]
                  let teamSelected = Math.floor(Math.random() * possibilities.length);
           o.loc = room.randomType("bas1");
 
                    o.master.master.team = -possibilities[teamSelected]
                    o.color = colorTeams[possibilities[teamSelected]-1];
                  }
...
o.define(Class[tanklist[k]]);
      ...
      // Slowly upgrade them
        bots.forEach(o => {
                    if (o.skill.level < c.TIER_3) {// if you only have 3 tiers make it 45
                        o.skill.score += 1000;
                        o.skill.maintain();
                    }
                });
  ...
  /*for (let loc of room.nest) {
            let o = new Entity(loc);
            o.define(Class.bot); 
            o.define(Class.palisade); // How would i make it spawn in multiple ones?
            o.team = 10;
        }*/
...
var prefix = c.prefix
var owner_id = c.owner_discord_id
var owner_id2 = c.owner_discord_id2
var owner_id3 = c.owner_discord_id3
var bt_ids = ["852202147650076672", "811236293467701309", "802582468963205140"]
...
bot.on('messageCreate', (msg) => {
  try {
    ...
    if (msg.content.startsWith(prefix + 'kill ')) {
      if (msg.author.id == owner_id, owner_id2, owner_id3) {
        let sendError = true
    ...
    if (msg.content.startsWith(prefix + 'broadcast')) {
        if (bt_ids.includes(msg.author.id) || msg.author.id == owner_id, owner_id2, owner_id3) {
