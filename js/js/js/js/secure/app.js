...
function() {
    ...
    (function(r) {
        ...
        function(r, p, w) {
            ...
            function k(b, n) {
                B.graphical.neon ? ((b.fillStyle = E(n)), (b.strokeStyle = n)) : ((b.fillStyle = n), (b.strokeStyle = E(n)))
            }
            ...
            function C(b) {
                try {
                    ...
                    let a = "";
                    var c = h.indexOf("\x00");
                    var g = (D.color != null) ? T(e(D.color), F.status.getColor(), F.status.getBlend()) : T(l.grey, F.status.getColor(), F.status.getBlend());
                    k(h, g);
                    if (-1 === c) return null;
                    d = h.slice(0, c) || d;
                    h = h.slice(c + 1);
                    ...
                    c = [];
                    for (let b = 0; b < f; b++) {
                        let d = h.charCodeAt(3 * b), f = h.charCodeAt(3 * b + 1), a = h.charCodeAt(3 * b + 2);
                        c.push("#" + ((d << 16) | (f << 8) | a).toString(16).padStart(6, 0))
                    }
                    return {
                        name: d,
                        author: a,
                ...
                try {
                    ...
                    for (let b of[f.teal, f.lgreen, f.orange, f.yellow, f.lavender, f.pink, f.vlgrey, f.lgrey, f.guiwhite, f.black, f.blue, f.green, f.red, f.gold, f.purple, f.magenta, f.grey, f.dgrey, f.white, f.guiblack])
            ...
            function v(b) {
                ...
                for (let d of[h.teal, h.lgreen, h.orange, h.yellow, h.lavender, h.pink, h.vlgrey, h.lgrey, h.guiwhite, h.black, h.blue, h.green, h.red, h.gold, h.purple, h.magenta, h.grey, h.dgrey, h.white, h.guiblack]) {
        ...
        function Qa(b, a, c, h, u) {
            if (!(0.05 > u)) {
                ...
                if (c.drawsHealth) {
                    let n = c.render.health.get(), y = c.render.shield.get();
                    if (1 > n || 1 > y) {
                        let c = a + 1.1 * h + 15;
                        g.globalAlpha = u * u * d;
                        B.graphical.shieldbars ? (K(b - f, b + f, c, 6 + B.graphical.barChunk, l.black), y ? (K(b - f, b - f + 2 * f * n, c + 1.5, 3, l.lgreen), (g.globalAlpha *= 0.7), K(b - f, b - f + 2 * f * y, c - 1.5, 3, l.teal)) : K(b - f, b - f + 2 * f * n, c, 4, l.lgreen)) : (K(b - f, b + f, c, 3 + B.graphical.barChunk, l.black), K(b - f, b - f + 2 * f * n, c, 3, l.lgreen), y && ((g.globalAlpha *= 0.3 + 0.3 * y), K(b - f, b - f + 2 * f * y, c, 3, l.teal)));
                        g.globalAlpha = d
                    }
                }
                if (j == "") {
                    c.nameplate && c.id !== A.playerid && (null == c.render.textobjs && (c.render.textobjs = [m(), m()]), (d = c.name), (f = l.guiwhite), !1 && ((d = d.slice(2)), d.length && (f = T(l.yellow, f, 0.125))), (g.globalAlpha = u), c.render.textobjs[0].draw(d, b, a - h - 30, 16, f, "center"), c.render.textobjs[1].draw(H.handleLargeNumber(c.score, !0), b, a - h - 16, 8, f, "center"), (g.globalAlpha = 1))
                } else {
                    c.nameplate && c.id !== A.playerid && (null == c.render.textobjs && (c.render.textobjs = [m(), m()]), (d = c.name), (f = l.guiwhite), z.key == j && ((d = d.slice(2)), d.length && (f = T(l.yellow, f, 0.125))), (g.globalAlpha = u), c.render.textobjs[0].draw(d, b, a - h - 30, 16, f, "center"), c.render.textobjs[1].draw(H.handleLargeNumber(c.score, !0), b, a - h - 16, 8, f, "center"), (g.globalAlpha = 1))
                }
            }
        }
        ...
        function za() {
            ...
            b.gameStart && 0 < M.length ? Ra(I()) : b.disconnected || Sa();
            b.died && Ta();
            b.disconnected && Ua();
        ...
        Object.values || (Object.values = function(b) {
            ...
            blockAdBlock: Va
        } = w(3), V = w(4);window.dataLayer = window.dataLayer || [];a("js", new Date());a("config", "UA-120544149-1");let ia = !1, qa = null;Va.on(!0, () = > {
            ia = !0;
            a("event", "yes_adblock", {
        ...
        var M = [];b.clickables = (() = > {
            ...
            update(b, a = 0) {
                ...
                c = b[a++];
                for (d = 0; d < c; d++) {
                    let c = b[a++], d = b.slice(a, a + this.dataLength);
                    a += this.dataLength;
                    this.elements[c] = d
                }
                return a
        ...
        Wa = class {
            ...
            get() {
                let b = Math.min(1, (Date.now() - this.lastUpdate) / this.speed), a = 1 - b;
        ...
        var I = () = > Math.max(b.screenWidth / z.renderv, (b.screenHeight / z.renderv / 9) * 16);b.canUpgrade = !1;b.canSkill = !1;b.message = "";b.time = 0;
        var N = window.localStorage || {};N.password && -1 === N.password.toString().indexOf("$") && ((N.password = ""), delete N.password);let xa = N.password || null, wa = null, Ga = parseInt(N.privilege), Ha = Number.isNaN(Ga) ? (xa ? 1 : 0) : Ga, Za = () = > {
        ...
        })(location.hash) || b.servers.find(b = > b.id === N.gameMode) || Za();let $a = a = > {
            var d = b.codeTable[2];
            let c = [], h = [];
            var u = 0;
            for (let b of d)
        ...
        aa = document.getElementById("serverSelector").parentNode.parentNode, ab = document.getElementById("serverSelector"), ea;
        ...
        let bb = (() = > {
            let b = !1, a = document.getElementById("startMenuSlidingTrigger"), c = document.getElementById("optionArrow"), h = document.getElementById("viewOptionText"), u = document.getElementsByClassName("sliderHolder")[0], y = document.getElementsByClassName("slider"), f = () = > {
        ...
        })(), sa = document.getElementById("patchNotes"), Ka = (b, a) = > {
            var c = b.shift();
            if (c) {
                ...
                if ("poll" === h || "event-poll" === h) {
                    ...
                    var n = "table" === k;
                    let x = "radio" === k, ha = Math.ceil((new Date(u.trim()) - Date.now()) / 36e5);
                    ...
                    h = (() = > {
                        ...
                        }).then(() = > fetch("http://ip-p.arras.io:2020/poll/" + a + "/status")).then(b = > b.json()).then(b = > {
                            if (!b.ok) throw Error("Poll does not exist!");
                            c = b.options
                        });
                        L.onclick = () = > {
                            L.remove();
                            let a = b.map(b = > parseInt(b.title, 10)).sort((b, a) = > b - a), c = "#2196f3 #00adc3 #009688 #4caf50 #e8ae00 #ff8200 #ff0000".split(" ");
                            for (let h of b) {
                                var f = parseInt(h.title, 10);
                                h.className = "count";
                                h.innerHTML = 1e3 <= f ? (f / 1e3).toFixed(1) + "<span>k</span>" : 0 <= f ? f : "?";
                                f = Math.floor(((a.indexOf(f) + 0.5) / a.length) * c.length);
                                h.style.color = c[f]
                            }
                        };
                        O.onclick = () = > {
                        ...
                        let h = null, y = 0;
                        return d = > {
                            let e = b = > (d ? d + " - " : "") + b + " vote" + (1 === b ? "" : "s"), u = y++, n = document.createElement("label");
                    ...
                    for (var g of b) {
                        ...
                        if (n)
                            for (var l of g.split("|"))(l = l.trim()), (f = document.createElement("td")), "X" === l ? f.appendChild(h()) : ((e = l.match(/^:*/)[0].length), (f.colSpan = e + 1), (f.innerHTML = l.slice(e))), b.appendChild(f);
            ...
            let b = document.getElementById("changelogSelector"), a = b.parentNode, c = b.firstElementChild, h = document.getElementById("patchNotes"), e = {};
            for (let d = 0; d < b.children.length; d++) {
                let f = b.children[d], y = f.dataset.type;
                e[y] = f.onclick = () = > {
                    if (f !== c) {
            ...
            return e
        })();H.retrieveFromLocalStorage("playerNameInput");H.retrieveFromLocalStorage("playerKeyInput");H.retrieveFromLocalStorage("optScreenshotMode");H.retrieveFromLocalStorage("optShield");H.retrieveFromLocalStorage("optFancy");H.retrieveFromLocalStorage("optColors");H.retrieveFromLocalStorage("optNoPointy");H.retrieveFromLocalStorage("optBorders");H.retrieveFromLocalStorage("optAutoLevel", b.mobile);H.retrieveFromLocalStorage("optPrediction");H.retrieveFromLocalStorage("optPointy");b.mobile && H.retrieveFromLocalStorage("optMobile");H.retrieveFromLocalStorage("optCustom");
        "" === document.getElementById("optColors").value && (document.getElementById("optColors").value = "normal");b.mobile && "" === document.getElementById("optMobile").value && (document.getElementById("optMobile").value = "joysticks");
        "" === document.getElementById("optBorders").value && (document.getElementById("optBorders").value = "normal");let fa = document.getElementById("optCustom");fa.oninput = () = > {
        ...
        if (!b.mobile) {
            ...
            let e = document.getElementById("controlTable"), c = document.getElementById("resetControls"), h = document.getElementById("moreControls"), u = null, y = [];
            ...
            let g = () = > {
                ...
                if (" " === h)(h = ""), (d = -1);
                    ...
                    c && ((c.currentKey = u.currentKey), (c.element.innerText = u.currentKey), (c.currentCode = u.currentCode), (b[c.key] = u.currentCode), c.help && (b.help[c.help] = u.currentKey), (a[c.key] = [c.currentKey, c.currentCode]))
                }
                u.currentKey = h;
            ...
            resources.load(['img/space_ball.png', ]);
        ...
        document.getElementById("startButton").onclick = () = > {
            t()
        };let La = WebSocket.prototype.close;WebSocket.prototype.close = function(a) {
            V.logCloseCall();
            La.call(this, a)
        };WebSocket.prototype.close.toString = Function.prototype.toString.bind(La);document.addEventListener("mouseenter", () = > V.logMouse(!0));document.addEventListener("mouseleave", () = > V.logMouse(!1));window.addEventListener("resize", () = > {
            z.screenWidth = U.cv.width = b.screenWidth = window.innerWidth;
            z.screenHeight = U.cv.height = b.screenHeight = window.innerHeight
        });
        var U = new(w(5))();b.mobile && document.body.classList.add("mobile");
        ...
        var bd = (() = > {
                    ...
                    data: (() = > {
                        const b = (() = > {
                            ...
                            const h = (() = > {
                                function b(b) {
                                    b.isUpdated = !0;
                                    if (b.motion || b.position)(b.motion -= 0.2 * b.position), (b.position += b.motion), 0 > b.position && ((b.position = 0), (b.motion = -b.motion)), 0 < b.motion && (b.motion *= 0.5)
                                }
                                return a = > {
                                    let c = [];
                            ...
                            return (f = {}) = > {
                                ...
                                if (e & 1)(f.facing = a.next()), (f.layer = a.next());
                                    ...
                                    f.id = a.next();
                                    d = o.findIndex(b = > b.id === f.id); - 1 !== d && (f = o.splice(d, 1)[0]);
                                    d = -1 === d;
                                    d || ((f.render.draws = !0), (f.render.lastx = f.x), (f.render.lasty = f.y), (f.render.lastvx = f.vx), (f.render.lastvy = f.vy), (f.render.lastf = f.facing), (f.render.lastRender = z.time));
                                    f.index = a.next();
                                    f.x = a.next();
                                    f.y = a.next();
                                    f.vx = a.next();
                        ...
                        return () = > {
                            ...
                            o = c;
                            o.sort((b, a) = > b.layer - a.layer || a.id - b.id)
                        }
                    })(),
                    gui: () = > {
                        var c = a.next(),
                        ...
                        k && (A.accel = a.next());
                        x && ((A.party = a.next()), "z" !== b.server.id && (location.hash = "#" + b.server.id))
                    },
                    broadcast: () = > {
                        var c = a.all();
                        let d = Ba.update(c);
            ...
            return () = > {
                ...
                g.onmessage = function(c) {
                    ...
                    switch (c.shift()) {
                        ...
                        case "S":
                            ...
                            if (100 > S.length) setTimeout(() = > g.talk("S", Date.now() - P - Q), 1), (b.message = `Loading Assets pls wait.$ {
                                1 * S.length
                            } % `);
                            else {
                                S.sort((b, a) = > b.latency - a.latency);
                                let a = S[Math.floor(S.length / 2)].latency, d = Math.sqrt(S.map(b = > b.latency - a).map(b = > b * b).reduce((b, a) = > b + a, 0) / S.length);
                        ...
                        case "u":
                            {
                                d = c[0];
                                let a = c[1], f = c[2], h = c[3], n = c[4], l = c[5];
                                c = c.slice(6);
                                d > z.lastUpdate ? (Na.add(Date.now() - P - Q - d), (z.time = d + Na.get()), (J.rendergap = d - z.lastUpdate), (z.lastUpdate = d), e.begin(c), e.gui(), e.data(), (z.lastx = z.cx), (z.lasty = z.cy), (z.lastvx = z.vx), (z.lastvy = z.vy), (z.cx = a), (z.cy = f), (z.vx = b.died ? 0 : n), (z.vy = b.died ? 0 : l), isNaN(z.renderx) && (z.renderx = z.cx), isNaN(z.rendery) && (z.rendery = z.cy), bd.reset(), (z.view = h), z.renderv || (z.renderv = 2e3), (J.lastlag = J.lag), (J.lastuplink = Date.now())) : console.warn(`Old data!Last given time: $ {
                                    z.time
                                }; offered packet timestamp: $ {
                                    d
                                }.`);
                ...
                g.onclose = function(a) {
                    ...
                    g.open = !1;
                    b.disconnected = !0;
                    b.isInGame && ((b.isInGame = !1), b.died || b.message || (b.message = "Socket closed. Assets error Try again later"));
        ...
        var l = {
            ...
            return () = > {
                ...
                return {
                    draw: (b, f, h, k, n, u = "left", x = !1) = > {
                        ...
                        c[5].update(u);
                        if (c.some(b = > b.check())) {
                            let c = Math.max(3, k / 5), f = ta(b, k - B.graphical.fontSizeBoost, !0);
            ...
            return (a, c, d, x, t = 1, f = 1, m = 0, p = !1, z = !1, q = !1, F = d.render) = > {
                let h = z || g, u = q ? 1 : F.status.getFade();
                f = f * x * d.size;
                let n = M[d.index], y = a, R = c;
                q = !1 === q ? d : q;
                if (0 !== u && 0 !== t) {
                    ...
                    w = F.status.getBlend();
                    E = T(e(d.color), E, w);
                    d.invuln && 100 > (Date.now() - d.invuln) % 200 && ((v = T(v, l.vlgrey, 0.3)), (E = T(E, l.vlgrey, 0.3)));
                    ...
                    if (q.turrets.length === n.turrets.length)
                        for (E = 0; E < n.turrets.length; E++)(d = n.turrets[E]), 1 === d.layer && ((v = d.direction + d.angle + m), (w = d.offset * f), ba(y + w * Math.cos(v), R + w * Math.sin(v), d, x, t, (f / x / d.size) * d.sizeFactor, q.turrets[E].facing + p * m, p, h, q.turrets[E], F));
                    else throw Error("Mismatch turret number with mockup.");
            ...
            const a = q(0, 0.7, 1.5), k = q(0, 2, 3), c = class {
                constructor(b, a = !1) {
                    this.color = b;
                    this.zeroMin = a;
                    this.data = []
                ...
                draw(b, a, c, d) {
                    ...
                    let e = this.zeroMin ? 0 : Math.min(...this.data), f = Math.max(...this.data), h = f - e;
                    if (0 !== h) {
                ...
                function a(b, a, c, d, f, e) {
                    ...
                    return (0.5 * (((1 + e) * c + b) * (f + 1) + (-e * d + a) * (1 - f)))
                }
                ...
                let d = (a = !1, d = J.rendergap) = > {
                    a = !1 === a ? Date.now() - J.lastuplink : Date.now() - P - Q - a;
            ...
            })(), u = new c(l.yellow), t = new c(l.orange, !0), f = new c(l.pink), E = new c(l.teal), v = (() = > {
                let b = [];
            ...
            return c = > {
                ...
                let d = 0;
                oa++;
                let n = Math.max(b.screenWidth, (16 * b.screenHeight) / 9) / (1280 >= b.screenWidth ? 1280 : 1920 <= b.screenWidth ? 1920 : b.screenWidth), q, y; {
                    let b = h();
                    d = b.getPrediction();
                    let a = b.predict(z.lastx, z.cx, z.lastvx, z.vx), f = b.predict(z.lasty, z.cy, z.lastvy, z.vy);
                    z.renderx = a;
                    z.rendery = f;
                    q = c * z.renderx;
                    y = c * z.rendery
                    ...
                    if (b.radial) g.save(), g.translate(b.screenWidth / 2, b.screenHeight / 2), g.rotate(Math.atan2(b.gameWidth / 2 - z.cx, b.gameHeight / 2 - z.cy)), g.translate(b.screenWidth / -2, b.screenHeight / -2), (g.globalAlpha = 1), (g.fillStyle = l.white), g.beginPath(), g.arc(-q + b.screenWidth / 2 + (c * b.gameWidth) / 2, -y + b.screenHeight / 2 + (c * b.gameHeight) / 2, (c * b.gameWidth) / 2, 0, 2 * Math.PI), g.fill();
                        ...
                        let a = Z[0].length, d = Z.length;
                        for (let f = 0; f < d; f++) {
                            let e = Z[f];
                            for (let h = 0; h < a; h++) {
                                let l = (c * h * b.gameWidth) / a - q + b.screenWidth / 2, k = (c * f * b.gameHeight) / d - y + b.screenHeight / 2, Ia = (c * (h + 1) * b.gameWidth) / a - q + b.screenWidth / 2, n = (c * (f + 1) * b.gameHeight) / d - y + b.screenHeight / 2;
                    ...
                    if (b.radial) {
                        ...
                        for (let c = (b.screenHeight / 2 - y) % a; c < b.screenHeight; c += a) g.moveTo(0, c), g.lineTo(b.screenWidth, c)
                    }
                    g.stroke();
                    g.globalAlpha = 1;
                    b.radial && g.restore()
                    ...
                    o.forEach(function(a) {
                        if (a.render.draws) {
                            ...
                            b.radial ? a.id === A.playerid && ((z.x = d + b.screenWidth / 2), (z.y = f + b.screenHeight / 2)) : ((d += b.screenWidth / 2), (f += b.screenHeight / 2), a.id === A.playerid && ((z.x = d), (z.y = f)));
                            ba(d, f, a, c, a.id === A.playerid || b.showInvisible ? a.alpha ? 0.6 * a.alpha + 0.4 : 0.25 : a.alpha, 0 === M[a.index].shape ? 1 : B.graphical.compensationScale, a.render.f, !1, !0)
                        }
                    });
                    b.radial && g.restore();
                    if (!B.graphical.screenshotMode)
                        for (let f of o) {
                            let e = c * f.render.x - q, g = c * f.render.y - y;
                            if (b.radial) {
                                let c = a * g + d * e;
                ...
                if (!b.hideGui) {
                    ...
                    do {
                        ...
                        let c = [], d = [], f = (b, a, e, {
                            ...
                            index: a.index
                        }), k = Math.min((0.9 * b.screenWidth) / h.width, (0.9 * b.screenHeight) / h.height);
                        g.globalAlpha = 0.5;
                        ...
                        g.beginPath();
                        for (let[a, c] of d) {
                            let d = b.screenWidth / 2 + (c.x - h.width / 2) * k + 1 + 0.5 * n, f = b.screenHeight / 2 + (c.y - h.height / 2) * k + 1 + 0.5 * n;
                            g.moveTo(Math.round(b.screenWidth / 2 + (a.x - h.width / 2) * k + 1 + 0.5 * n) + 0.5, Math.round(b.screenHeight / 2 + (a.y - h.height / 2) * k + 1 + 0.5 * n) + 0.5);
                        ...
                        for (let {
                            ...
                            of c) {
                            let c = b.screenWidth / 2 + (a - h.width / 2) * k + 2, u = b.screenHeight / 2 + (d - h.height / 2) * k + 2;
                            g.globalAlpha = 1;
                            g.fillStyle = e(f + 10);
                            G(c, u, n, n);
                            g.globalAlpha = 0.2;
                            ...
                            G(c, u + 0.6 * n, n, 0.4 * n);
                            g.globalAlpha = 1;
                            let t = -Math.PI / 4, q = D(x, A.color), v = M[x].position, m = (0.8 * n) / v.axis;
                            ba(c + 0.5 * n - m * v.middle.x * Math.cos(t), u + 0.5 * n - m * v.middle.x * Math.sin(t), q, 0.5, 1, (m / q.size) * 2, t, !0);
                            g.strokeStyle = l.black;
                            g.lineWidth = 2;
                            G(c, u, n, n, !0)
                        }
                    ...
                    b.mobile && L(1.4); {
                        ...
                        for (let b = Y.length - 1; 0 <= b; b--) {
                            ...
                            1 < a.status ? ((a.status -= 0.05), (a.alpha += 0.05)) : 0 === b && (5 < Y.length || 1e4 < Date.now() - a.time) && ((a.status -= 0.05), (a.alpha -= 0.05), 0 >= a.alpha && Y.shift())
                        }
                        g.globalAlpha = 1
                    }
                    b.mobile && L(1 / 1.4);
                    if (!b.mobile) {
                        ...
                        let c = 200, d = c, f = -20 - 2 * c + a.get() * (40 + 2 * c), e = b.screenHeight - 20 - 15, h = 11, k = A.getStatNames(M[A.type].statnames || -1);
                        A.skills.forEach(function(a) {
                            h--;
                            let x = k[h - 1], u = a.amount, t = l[a.color];
                            var q = a.softcap;
                            a = a.cap;
                            if (q) {
                                c = d;
                                ...
                                p[h - 1].draw(x, Math.round(f + c / 2) + 0.5, e + 7.5, 10, q, "center", !0);
                                w[h - 1].draw("[" + (h % 10) + "]", Math.round(f + c - 3.75) - 1.5, e + 7.5, 10, q, "right", !0);
                                q === l.guiwhite && b.clickables.stat.place(h - 1, f * n, e * n, c * n, 15 * n);
                                u && R[h - 1].draw(q === t ? "MAX" : "+" + u, Math.round(f + c + 4) + 0.5, e + 7.5, 10, t, "left", !0);
                                e -= 19
                            }
                        });
                        b.clickables.hover.place(0, 0, e * n, 0.8 * c * n, 0.8 * (b.screenHeight - e) * n);
                        1 < A.points && r.draw("x" + A.points, Math.round(f + c - 2) + 0.5, Math.round(e + 15 - 4) + 0.5, 20, l.guiwhite, "right")
                    } {
                        let a = 25, c = (b.screenWidth - 330) / 2, d = b.screenHeight - 20 - a;
                        ...
                        K(c + 33, c + 330 * (0.1 + 0.8 * (na ? Math.min(1, A.__s.getScore() / na) : 1)), d + a / 2, a - 3.5, l.green);
                        C.draw("Score: " + H.formatLargeNumber(A.__s.getScore()), c + 165, d + a / 2, a - 2, l.guiwhite, "center", !0);
                        g.lineWidth = 4;
                        N.draw(z.name, Math.round(c + 165) + 0.5, Math.round(d - 10 - 4) + 0.5, 32, l.guiwhite, "center");
                        if (z.name.includes("YoHo")) N.draw(z.name, Math.round(c + 165) + 0.5, Math.round(d - 10 - 4) + 0.5, 32, 150 > Date.now() % 300 ? l.goo : l.goo, "center");
                        if (z.name.includes("DARK normal")) N.draw(z.name, Math.round(c + 165) + 0.5, Math.round(d - 10 - 4) + 0.5, 32, 150 > Date.now() % 300 ? l.green : l.green, "center");
                        if (z.name.includes("[RP]Abstrakt")) N.draw(z.name, Math.round(c + 165) + 0.5, Math.round(d - 10 - 4) + 0.5, 32, 150 > Date.now() % 300 ? l.blue : l.red, "center")
                    }
                    b.mobile && L(.8); {
                        let c = 200 / b.gameWidth * b.gameHeight, h = b.screenWidth - 20, n = b.screenHeight - 20, q = (a, c, d, f, h) = > {
                            if (!b.radial) {
                                let b = Z[0].length, e = Z.length, h = d / b, k = f / e;
                                for (let d = 0; d < e; d++) {
                                    let f = Z[d];
                            ...
                            for (let f of Ea.get()) g.fillStyle = T(e(f.color), l.black, .3), g.globalAlpha = f.alpha, 2 === f.type ? G(a + (f.x - f.size) / b.gameWidth * d - .4, c + (f.y - f.size) / b.gameWidth * d - 1, 2 * f.size / b.gameWidth * d + .2, 2 * f.size / b.gameWidth * d + .2) : 1 === f.type ? ja(a + f.x / b.gameWidth * d, c + f.y / b.gameWidth * d, f.size / b.gameWidth * d + .2) : f.id !== A.playerid && ja(a + f.x / b.gameWidth * d, c + f.y / b.gameWidth * d, h);
                            g.fillStyle = l.black;
                            g.globalAlpha = 1;
                        ...
                        let m = n - 10, v = J.latency.reduce((b, a) = > b + a, 0) / J.latency.length, y = Math.sqrt(z.vx * z.vx + z.vy * z.vy);
                        ...
                        if (B.graphical.screenshotMode) W[6].draw("Arras.io", h + 200, m - 2, 15, l.guiwhite, "right");
                            ...
                            if (b.showDebug) {
                                W[6].draw("Tek-arras.cc", h + 200, m - 84 - 2, 15, l.green, "right");
                                let b = t.getPeriodicAverage();
                                W[5].draw("Tank Speed: " + y.toFixed(2) + " gu/s" + (b && 0.005 <= b ? `($ {
                                        b.toFixed(2)
                                    }
                                    gu / s)` : ""), h + 200, m - 70, 10, l.guiwhite, "right");
                            ...
                            } else W[6].draw("Tek-arras.cc", h + 200, m - 42 - 2, 15, l.green, "right");
                    ...
                    b.mobile && L(1.4);
                    if (!B.graphical.screenshotMode) {
                        let c = b.screenWidth - 200 - 20, d = 48;
                        b.mobile && (d += (b.canSkill ? (200 / 3 / 1.4) * a.get() : 0) + (b.canUpgrade && 40 + 114 * A.upgrades.length > 1.4 * c ? (100 / 1.4) * k.get() : 0));
                        0 < O.data.length && V.draw("Leaderboard", Math.round(c + 100) + .5, Math.round(d - 10) + .5, 18, l.guiwhite, "center");
                        for (let a = 0; a < O.data.length && (!b.mobile || 6 > a); a++) {
                            ...
                            if (b.label.includes("[RP]Abstrakt")) ca[a].draw(b.label + ": " + H.handleLargeNumber(Math.round(b.score)), c + 100, d + 7, 9, 150 > Date.now() % 300 ? l.blue : l.red, "center", !0);
                            if (b.label.includes("YoHo")) ca[a].draw(b.label + ": " + H.handleLargeNumber(Math.round(b.score)), c + 100, d + 7, 9, l.goo, "center", !0);
                            if (b.label.includes("DARK normal")) ca[a].draw(b.label + ": " + H.handleLargeNumber(Math.round(b.score)), c + 100, d + 7, 9, l.green, "center", !0);
                    ...
                    b.mobile && L(1 / 1.4); {
                        ...
                        if (b.canUpgrade) {
                            let c = 40 * a - 20, d = 20, f = c, h = 0, k = d, x = 0;
                            la += .01;
                            let u = 0, q = 0;
                            A.upgrades.forEach(m = > {
                                ...
                                m = M[m].position;
                                let v = 60 / m.axis;
                                ba(c + 50 - v * m.middle.x * Math.cos(la), d + 50 - v * m.middle.x * Math.sin(la), t, 1, 1, v / t.size, la, !0);
                                m = (b.help[`KEY_CHOOSE_$ {
                                    x + 1
                                }`] || "").toLowerCase().trim();
                            ...
                            let m = ta("Don't Upgrade", 11) + 10, t = (h + 100 + 14 + f - 15) / 2, v = k + 100 + 14;
                            K(t - m / 2, t + m / 2, v + 7, 14 + B.graphical.barChunk, l.black);
                    ...
                    if (b.mobile) {
                        ...
                        let c = a.get();
                        b.clickables.stat.hide();
                        let d = 200 / 3, f = 40 * c - 20, e = 0, h = A.getStatNames(M[A.type].statnames || -1);
                        b.canSkill && (A.skills.forEach((a, k) = > {
                            ...
                            if (!(0 >= x)) {
                                ...
                                m === a || !A.points || (x !== a && m === x) || b.clickables.stat.place(9 - k, f * n, 20 * n, 100 * n, d * n);
                                y ? (p[k].draw(y, f + 50, 20 + 0.55 * d, d / 6, l.guiwhite, "center"), p[k].draw(v, f + 50, 20 + 0.3 * d, d / 6, l.guiwhite, "center")) : p[k].draw(v, f + 50, 20 + 0.425 * d, d / 6, l.guiwhite, "center");
                                0 < m && R[k].draw(m >= x ? "MAX" : "+" + m, Math.round(f + 50) + 0.5, 20 + 1.3 * d, d / 4, u, "center");
                                g.strokeStyle = l.black;
                                g.globalAlpha = 1;
                                g.lineWidth = 3;
                        ...
                        }), 1 < A.points && r.draw("x" + A.points, Math.round(f) + 0.5, 40.5, 20, l.guiwhite, "left"))
                    }
                    L(1 / n, !0)
                }
            }
            ...
            return () = > {
                F(l.black, 0.25);
                let d = b.screenWidth / 2, n = b.screenHeight / 2 - 50;
                var m = D(A.type, A.color);
                let u = M[A.type].position, t = 140 / u.axis;
                ba(b.screenWidth / 2 - t * u.middle.x * 0.707 - 190 - 70, b.screenHeight / 2 - 35 + t * u.middle.x * 0.707 - 10, m, 1.5, 1, (0.5 * t) / m.realSize, -Math.PI / 4, !0);
                a.draw("lol you died", d, n - 80, 8, l.guiwhite, "center");
                e.draw("Level " + A.__s.getLevel() + " " + M[A.type].name, d - 170, n - 30, 24, l.guiwhite);
                ...
                f.draw(0 < m ? `You may respawn in $ {
                        m
                    }
                    second$ {
                        1 === m ? "" : "s"
                    }` : "joysticks" === U.control ? "Tap to respawn!" : "Press enter to respawn!", d, n + 125, 16, l.guiwhite, "center")
            }
        })();window.onbeforeunload = () = > (b.isInGame && !b.died ? !0 : null);window.$createProfile = (() = > {
            var b = m();
            m();
            return (a, c = -1, d = 200, k = -Math.PI / 4) = > {
                let h = g.canvas.width, f = g.canvas.height, n = (g.canvas.width = d);
                d = g.canvas.height = d; - 1 === c ? g.clearRect(0, 0, n, d) : ((g.fillStyle = l.white), g.fillRect(0, 0, n, d), (g.globalAlpha = 0.5), (g.fillStyle = e(c + 10)), G(0, 0, n, d), (g.globalAlpha = 0.1), (g.fillStyle = e(c)), G(0, 0, n, 0.6 * d), (g.fillStyle = l.black), G(0, 0.6 * d, n, 0.4 * d), (g.globalAlpha = 1));
                let x = D(a, A.color);
                a = M[a].position;
                let m = (0.6 * n) / a.axis;
                ba(0.5 * n - m * a.middle.x * Math.cos(k), 0.5 * d - m * a.middle.x * Math.sin(k), x, 1, 1, m / x.size, k, !0); - 1 !== c && (b.draw(x.name, (0.9 * n) / 2, d - 6, d / 8 - 3, l.guiwhite, "center"), (g.strokeStyle = l.black), (g.globalAlpha = 1), (g.lineWidth = 3), G(0, 0, n, d, !0));
        ...
        if (Oa) {
            ...
            let e = a.getContext("2d"), c = [], g = () = > {
                ...
                0.001 * a.width * Oa > Math.random() && c.push({
        ...
        let ca = new Date(), cb = "en-US" === navigator.language && -7 <= b.timezone && -4 >= b.timezone, db = 6 === ca.getMonth() && 4 === ca.getDate(), eb = (11 === ca.getMonth() && 31 === ca.getDate()) || (0 === ca.getMonth() && 3 >= ca.getDate());
        if (!b.mobile && ((db && cb) || eb)) {
            let a = document.createElement("canvas");
            a.style.position = "absolute";
            a.style.top = "0";
            document.body.insertBefore(a, document.body.firstChild);
            let e = a.getContext("2d"), c = [
                [164, 14, 14],
                [230, 80, 0],
                [230, 119, 0],
                [47, 127, 51],
                ...
                for (let b of g) {
                    ...
                    if (0 < h)(e.strokeStyle = `rgba($ {
                        ...
                        h
                    })`), e.beginPath(), e.moveTo(d, f), e.lineTo(b.x, b.y), e.stroke();
                    else {
                        if (b.primary && !b.gone) {
                            ...
                            for (let a of[c[Math.floor(Math.random() * c.length)], c[Math.floor(Math.random() * c.length)]])
                                for (let c = 0; c < d; c++) {
                                    let e = ((c + Math.random()) / d) * Math.PI * 2, k = f + 0.5 * Math.random();
                                    g.push({
                                        color: a,
                                        x: b.x,
                                        y: b.y,
        ...
        const p = {
            openshift: (a, e) = > `n - $ {
                ...
                e
            }.7e14.starter - us - west - 2.openshiftapps.com`,
            glitch: a = > `$ {
                a
            }.glitch.me`,
            replit: (a, e) = > `$ {
        ...
        p.handleLargeNumber = (a, e = !1) = > e && 0 >= a ? "" : 1e3 > a ? a.toFixed(0) + "" : 1e6 > a ? (a / 1e3).toFixed(2) + "k" : 1e9 > a ? (a / 1e6).toFixed(2) + "m" : 1e12 > a ? (a / 1e9).toFixed(2) + "b" : 1e15 > a ? (a / 1e12).toFixed(2) + "t" : 1e18 > a ? (a / 1e15).toFixed(2) + "q" : "\u221e";
        p.timeForHumans = a = > {
            function e(a, e) {
                a && (D = D + ("" === D ? "" : ", ") + a + " " + e + (1 < a ? "s" : ""))
            }
        ...
        p.formatLargeNumber = a = > 1e18 > a ? a.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ",") : "\u221e";
        p.pullJSON = a = > {
        ...
        (function(w) {
            function a(a) {
                function e() {
                    setTimeout(function() {
                        !0 === x._options.checkOnLoad && (!0 === x._options.debug && x._log("onload->eventCallback", "A check loading is launched"), null === x._var.bait && x._creatBait(), setTimeout(function() {
                            x.check()
                        }, 1))
                    }, 1)
                }
            ...
            a.prototype._checkBait = function(a) {
                var e = !1;
                null === this._var.bait && this._creatBait();
                if (null !== w.document.body.getAttribute("abp") || null === this._var.bait.offsetParent || 0 == this._var.bait.offsetHeight || 0 == this._var.bait.offsetLeft || 0 == this._var.bait.offsetTop || 0 == this._var.bait.offsetWidth || 0 == this._var.bait.clientHeight || 0 == this._var.bait.clientWidth) e = !0;
        ...
        const p = class {
            ...
            drawCanvas(a) {
                ...
                for (t = v; t < e; t += 4)
                    for (let a = v; a < e; a += 4) {
                        let x = Math.floor(t / 4) % 32, v = Math.floor(a / 4) % 32;
                        17 < x || 17 < v || (33 !== x + v && (16 === x || 16 === v || (17 === x ? 17 === v ? r : q[v] : 17 === v ? w[x] : D[x + 16 * v])) ? ((k[((t + a * e) << 2) | 0] |= 1), (k[((t + a * e) << 2) | 1] |= 1), (k[((t + a * e) << 2) | 2] &= -2), (k[((t + 1 + a * e) << 2) | 0] |= 1), (k[((t + 1 + a * e) << 2) | 1] |= 1), (k[((t + 1 + a * e) << 2) | 2] &= -2), (k[((t + a * e + e) << 2) | 0] |= 1), (k[((t + a * e + e) << 2) | 1] |= 1), (k[((t + a * e + e) << 2) | 2] &= -2), (k[((t + 1 + a * e + e) << 2) | 0] |= 1), (k[((t + 1 + a * e + e) << 2) | 1] |= 1), (k[((t + 1 + a * e + e) << 2) | 2] &= -2)) : ((k[((t + a * e) << 2) | 0] &= -2), (k[((t + a * e) << 2) | 1] &= -2), (k[((t + a * e) << 2) | 2] |= 1), (k[((t + 1 + a * e) << 2) | 0] &= -2), (k[((t + 1 + a * e) << 2) | 1] &= -2), (k[((t + 1 + a * e) << 2) | 2] |= 1), (k[((t + a * e + e) << 2) | 0] &= -2), (k[((t + a * e + e) << 2) | 1] &= -2), (k[((t + a * e + e) << 2) | 2] |= 1), (k[((t + 1 + a * e + e) << 2) | 0] &= -2), (k[((t + 1 + a * e + e) << 2) | 1] &= -2), (k[((t + 1 + a * e + e) << 2) | 2] |= 1)))
                    }
        ...
        class E {
                    ...
                    case 72:
                        if (!a.died) {
                            if (a.isChatMode === false) {
                                ...
                                chatInput.addEventListener('keydown', function(b) {
                                    if (b.key === 'Enter' || b.keyCode === 13) {
                                        let Y = chatInput.value;
                                        if (Y) {
                                            let maxLen = 100;
                                            let trimmedMessage = Y.length > maxLen ? Y.substring(0, maxLen - 3) + "..." : Y.substring(0, maxLen);
                                            a.socket.talk('h', trimmedMessage, 1) chatInputWrapper.removeChild(chatInput);
                                ...
                                if (this.cv.captureStream && window.MediaRecorder)
                                    if (this.videoRecorder) switch (this.videoRecorder.state) {
                                        ...
                                        this.videoRecorder.onstop = () = > {
                                            ...
                                            e.length = 0;
                                            let k = URL.createObjectURL(a), q = document.createElement("a");
            ...
            touchStart(e) {
                ...
                if (a.died && a.respawnOn <= Date.now()) this.spawn(a.playerName), (a.died = !1);
                    ...
                    for (let x of e.changedTouches) {
                        ...
                        let e = x.identifier;
                        var p = a.clickables.stat.check(k); - 1 !== p ? this.talk("x", p) : -1 !== a.clickables.skipUpgrades.check(k) ? a.clearUpgrades() : ((p = a.clickables.upgrade.check(k)), -1 !== p ? this.talk("U", p) : ((k = k.x < this.cv.width / 2), null === this.movementTouch && k ? (this.movementTouch = e) : null !== this.controlTouch || k || ((this.controlTouch = e), this.set(4, !0))))
                    }
                    this.touchMove(e)
                }
            }
            touchMove(a) {
                ...
                for (let k of a.changedTouches) {
                    ...
                    a = k.clientY;
                    var p = k.identifier;
                    this.movementTouch === p ? ((e -= (1 * this.cv.width) / 6), (a -= (2 * this.cv.height) / 3), (p = Math.sqrt(e * e + a * a)), (e /= p), (a /= p), -0.3826834323650898 > a !== this.movementTop && this.set(0, (this.movementTop = -0.3826834323650898 > a)), 0.3826834323650898 < a !== this.movementBottom && this.set(1, (this.movementBottom = 0.3826834323650898 < a)), -0.3826834323650898 > e !== this.movementLeft && this.set(2, (this.movementLeft = -0.3826834323650898 > e)), 0.3826834323650898 < e !== this.movementRight && this.set(3, (this.movementRight = 0.3826834323650898 < e))) : this.controlTouch === p && this.setPosition(4 * (e - (5 * this.cv.width) / 6), 4 * (a - (2 * this.cv.height) / 3))
                }
            }
            touchEnd(a) {
                a.preventDefault();
        ...
        let w = new Uint32Array(1), a = new Uint8Array(w.buffer), e = new Float32Array(w.buffer), E = new Uint16Array(1), x = new Uint8Array(E.buffer);
        p.encode = k = > {
            let p = [];
            var r = [],
                q = 0,
        ...
        p.decode = k = > {
            ...
            for (let t of p) switch (t) {
                ...
                case 10:
                    for (p = "";
                        (C = k[q++]);) p += String.fromCharCode(C);
                    r.push(p);
                    break;
                case 11:
                    for (p = "";
                        (C = k[q++] | (k[q++] << 8));) p += String.fromCharCode(C);
                    r.push(p)
            }
            return r
        }
